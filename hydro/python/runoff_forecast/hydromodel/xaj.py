...
#@jit          (changed by Bill)
def calculate_prcp_runoff(b, im, wm, w0, pe, ped, nd):
    """
    ...
    """
    wmm = wm * (1.0 + b)/(1.0-im)     #changed by Bill(20220719),original code was wmm = wm * (1.0 + b)
...
def generation(p_and_e, k, b, im, um, lm, dm, c, wu0=None, wl0=None, wd0=None) -> tuple:
    ...
    #Newly added code for source-division are below(Bill)
    #ped = np.expand_dims(pe, axis=-1)
    #ped = np.full(pe.shape, 0.0)
    if pe <= 5:
        nd = 1    
        ped = pe
    else:
        ...
        for i in range(nd):
            ped[i] = 5
        ped[nd-1] = pe-(nd-1)*5+1e-5    
    #Newly added code for source-division are above(Bill)
    r, rim, rd = calculate_prcp_runoff(b, im, wm, w0, pe, ped, nd)
    # Update wu, wl, wd
    wu, wl, wd = calculate_w_storage(
        um, lm, dm, wu0, wl0, wd0, eu, el, ed, prcp_difference, r
    )

    return (r, rim, e, pe, ped, rd, nd), (wu, wl, wd)


def sources(pe, r, sm, ex, ki, kg, im, ped, rd, nd, s0=None, fr0=None) -> tuple:
    """
    ...
    if pe <= 0:
        ...
        s = s0
        for i in range(nd):
            td = rd[i] - im * ped[i]    #透水面积上的产流 = 时段产流量 - 不透水面积产流量   changed by Bill
            xx = fr                     #前一时段的产流面积比
...
def sources5mm(
    ...
    s0=None,
    fr0=None,
    time_interval_hours=1,#time period(changed by Bill)
    book="ShuiWenYuBao",
):
    """
    ...
    for j in range(n):
        ...
        if book == "ShuiWenYuBao":
            ...
            rg_j = s_d * kg_d * fr_d
            s_d = s_d * (1 - kss_d - kg_d)      #changed by Bill.Original code is s_d = s_d * (1 - rss_j + rg_j) 
...
def xaj(
    ...
    runoff_= np.full(inputs.shape[:2], 0.0)
    ...
    for i in range(inputs.shape[0]):
        if debug: print('p_and_e',inputs[i, :, :])
        if i == 0:
            (r, rim, e, pe, ped, rd, nd), w = generation(
                inputs[i, :, :], k, b, im, um, lm, dm, c, *w0
            )
            runoff_[i, :]= r
            e_[i, :]= e
            if source_type == "sources":
                (rs, ri, rg), (s, fr) = sources(pe, r, sm, ex, ki, kg, im, ped, rd, nd, s0, fr0) 
            ...
            e_[i, :]= e
            if source_type == "sources":
                (rs, ri, rg), (s, fr) = sources(pe, r, sm, ex, ki, kg, im, ped, rd, nd, s, fr)
            elif source_type == "sources5mm":
                (rs, ri, rg), (s, fr) = sources5mm(
        ...
        if debug: print('e_:%.4f; r:%.4f; rim:%.4f; pe:%.4f'%(e,r,rim,pe))
        if debug: print('w:',w)
        if debug: print("s:%.4f; fr:%.4f"%(s,fr))
        if debug: print("rs:%.4f; ri:%.4f; rg:%.4f"%(rs,ri,rg))
        if debug: print('*'*40)
        # impevious part is pe * im
        runoff_ims_[i, :] = rim
        # so for non-imprvious part, the result should be corrected
        rss_[i, :] = rs          #changed by Bill(20220179).oringinal code was rss_[i, :] = rs * (1 - im)
        ris_[i, :] = ri          #oringinal code was ris_[i, :] = ri * (1 - im)
        rgs_[i, :] = rg          #oringinal code was rgs_[i, :] = rg * (1 - im)
    # seq, batch, feature
    runoff_im = np.expand_dims(runoff_ims_, axis=2)
    rss = np.expand_dims(rss_, axis=2)

    if debug: print("="*50)
    if debug: print("qi0:%.4f; qg0:%.4f"%(qi0,qg0))
    ...
    if route_method == "CSL":
        ...
        for i in range(inputs.shape[0]):
            ...
            qt[i, :] = qs_ + qi + qg
            if debug: print("qs:%.4f; qi:%.4f; qg:%.4f; qt:%.4f"%(qs_,qi,qg,qt[i, :]))
    ...
    if return_state:
        return q_sim, *w, s, fr, qi, qg,runoff_[:],runoff_ims_[:],e_[:],s_[:],fr_[:],rss_,ris_,rgs_
        #0-q_sim; 1-um; 2-lm; 3-dm; 4-s; 5-fr; 6-qi; 7-qg; 8-runoff_[:]; 9-runoff_ims_[:];
        #10-e_[:]; 11-s_[:]; 12-fr_[:]; 13-rss_; 14-ris_; 15-rgs_;
    return q_sim
...
# delta_T: 单位时间，单位h
def ROdepth_to_Q(res,Area,delta_T): # mm^3/Dt/m^2 ~> m^3/s
    res_val = res[0]
    Q       = (res_val[: , :, 0] * Area) / 1000 / 3600 / delta_T
    return Q[:,0]

# 在经过新安江模型演算后最后一个时刻的土壤状态和流量
# delta_T: 单位时间，单位h
...
def calc_xaj(Rainfall:np.ndarray,Evaporation:np.ndarray,
    ...
    res = xaj(p_e,
            ...
            return_state=True,
            warmup_length=0)
    Q = ROdepth_to_Q(res,Area,Hydro_Dt)
